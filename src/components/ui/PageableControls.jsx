    ...
    <div className={styles.pagination__container}>
      {currentPage > 0 &&
        <button className={styles.pagination__btn}>
          <Link className={styles.pagination__link} href={`?page=${currentPage - 1}`}>
        ...
        {pageNumbers.map((page, index) => (
          <div>
            {page === '...' ? (
              ...
              <button key={index} className={styles.pagination__btn + (currentPage === page ? ` ${styles.pagination__current}` : '')} disabled={currentPage === page}>
                {
                  currentPage === page ? (
                    <span className={styles.pagination__link}>{page + 1}</span>
                  ) : (
                    <Link className={styles.pagination__link} href={`?page=${page}`}><span>{page + 1}</span></Link>
                  )
                }
              </button>
